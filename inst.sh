...
# Blue
functionshow_progress_info()
{
	echo $(tput setaf 4)$@$(tput sgr0)
}

# Red color
functionshow_progress_error()
{
...
END=$(date +%s) >> Times.log
apt-get -y --force-yes install mariadb-server
# Start timer again.
START2=$(date +%s) >> Times.log
...
END2=$(date +%s) >> Times.log
DIFF1=$(( $END - $START ))
DIFF2=$(( $END2 - $START2 ))
DIFF=$(( $DIFF1 + $DIFF2 ))
echo Total difference is $DIFF  >> Times.log
echo Hurray! In mere $(($DIFF / 60 )) minutes and $(($DIFF % 60 )) seconds all is finished! Congrats dude... >>TimeEcho.log
show_progress_info "$(cat TimeEcho.log)"
